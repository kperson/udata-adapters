...
class TestMongoManager(val manager: PubSubManager[Array[Byte]], database: DefaultDB) extends MongoPubSubManagerActor {
...
class MongoPubSubManagerActorSpec extends PubSubManagerActorSpec with MongoSpec {
  ...
  def withManager(manager: PubSubManager[Array[Byte]])(testCode: (ActorRef) => Unit) {
    val actorRef = system.actorOf(Props(new TestMongoManager(manager, database)))
    testCode(actorRef)
  }
